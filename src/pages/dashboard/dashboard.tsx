    ...
    <div className='p-6 py-2'>
      <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card title='Traslados pendientes' icon={DeliveryIcon} iconColor='text-yellow-500'>
          <p className="text-4xl font-bold text-yellow-500">{transfers?.filter(transfer => ["pendiente", "planificado", "confirmado", "en camino"].includes(transfer.status || "")).length || 0}</p>
      ...
      <TableContainer 
        transfers={transfers?.filter(transfer => ["pendiente", "planificado", "confirmado", "en camino"].includes(transfer.status || "")) ?? []} 
